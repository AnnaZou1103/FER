...
def get_args_parser():
    ...
    parser.add_argument('--distillation-alpha', default=4.0, type=float, help="")
    parser.add_argument('--distillation-beta', default=0.1, type=float, help="")
...
if __name__ == '__main__':
    ...
    file_dir = '../checkpoints/retina_dis'
    ...
    # teacher_model = swinv2_base_window16_256(pretrained=True)
    # teacher_model = Swin(swinv2_base_window16_256(pretrained=True))
    teacher_model = torch.load('../checkpoints/retina_base/best.pth')
    teacher_model.eval()
    teacher_model.to(DEVICE)

    criterion = SoftTargetCrossEntropy()
    criterion_train = DistillationLoss(criterion, teacher_model, args)
